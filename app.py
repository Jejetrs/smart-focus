...
def create_session_recording_from_frames(recording_frames, output_path):
    """Create video recording from captured frames with face landmarks and bounding boxes"""
    try:
...
def create_demo_recording_file():
    """Create a demo recording file for demonstration purposes"""
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
def cleanup_old_session_files():
    ...
    try:
        ...
        for folder_key in ['DETECTED_FOLDER', 'REPORTS_FOLDER', 'RECORDINGS_FOLDER']:
            folder_path = application.config[folder_key]
            if os.path.exists(folder_path):
                for filename in os.listdir(folder_path):
                    if filename.startswith(('session_', 'person_', 'processed_')):
...
def reset_session_data():
    ...
    print(f"Session data reset. New session ID: {session_id}")
...
def start_monitoring():
    ...
    try:
        # Cleanup old files and reset session data
        ...
        print(f"Monitoring started for session: {session_data['session_id']}")
...
def stop_monitoring():
    ...
    try:
        ...
        # Generate PDF report with new filename
        ...
        # Store PDF path in session data
        if pdf_result and os.path.exists(pdf_path):
            session_data['pdf_report_path'] = pdf_path
            response_data["pdf_report"] = f"/static/reports/{pdf_filename}"
            print(f"PDF report generated: {pdf_filename}")
        
        # Generate video recording with new filename
        recording_filename = f"session_recording_{session_id}.mp4"
        recording_path = os.path.join(application.config['RECORDINGS_FOLDER'], recording_filename)
        
        # Try to create recording from captured frames
        video_result = None
        if session_data.get('recording_frames') and len(session_data['recording_frames']) > 0:
            ...
            print(f"Creating video from {len(session_data['recording_frames'])} captured frames")
        else:
            # Create demo recording if no frames captured
            video_result = create_demo_recording_file()
            if video_result:
                recording_path = video_result
                recording_filename = os.path.basename(recording_path)
                print("Created demo recording
