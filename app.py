# app.py - Smart Focus Alert System
...
def init_mediapipe():
    ...
    try:
        ...
        print(f"MediaPipe failed: {str(e)}")
        return False
...
def model_detect(frame, landmarks):
    """Detect drowsiness with landmarks"""
    COLOR_RED = (0, 0, 255)
    COLOR_BLUE = (255, 0, 0)
    COLOR_GREEN = (0, 255, 0)
...
def update_distraction_sessions(person_id, current_state, current_time):
    """Update person state tracking"""
    global person_distraction_sessions, person_current_states, person_state_start_times
    global session_data, session_start_time, last_alert_times
    ...
    if person_key not in person_current_states:
        ...
        last_alert_times[person_key] = {}
    
    previous_state = person_current_states[person_key]
    
    if previous_state != current_state:
        print(f"Person {person_id} state: {previous_state} -> {current_state}")
        ...
        if previous_state and previous_state in DISTRACTION_THRESHOLDS:
            ...
            print(f"Closed {previous_state} session: {session_duration:.2f}s")
        
        person_current_states[person_key] = current_state
        person_state_start_times[person_key] = current_time
        
        if person_key not in last_alert_times:
            last_alert_times[person_key] = {}
        if current_state in last_alert_times[person_key]:
            del last_alert_times[person_key][current_state]
...
def should_trigger_alert(person_id, current_state, current_duration):
    ...
    if current_state not in last_alert_times[person_key]:
        print(f"First alert: Person {person_id} - {current_state} after {current_duration:.1f}s")
        return True, False
    else:
        time_since_last = current_time - last_alert_times[person_key][current_state]
        if time_since_last >= ALERT_COOLDOWN:
            print(f"Reminder alert: Person {person_id} - {current_state} ({current_duration:.1f}s total)")
            return True, True
    
    return False, False

def trigger_alert(person_id, alert_type, duration, is_reminder=False):
    """Trigger alert"""
    ...
    last_alert_times[person_key][alert_type] = current_time
    
    if alert_type == 'SLEEPING':
        base_message = f'Person {person_id} is sleeping - please wake up!'
    elif alert_type == 'YAWNING':
        base_message = f'Person {person_id} is yawning - please take a rest!'
    elif alert_type == 'NOT FOCUSED':
        base_message = f'Person {person_id} is not focused - please focus on screen!'
    else:
        return
    ...
    with monitoring_lock:
        if live_monitoring_active and session_data and session_data.get('start_time'):
            ...
            print(f"Alert stored - {display_message} (Duration: {duration:.1f}s)")

def calculate_distraction_times():
    """Calculate total distraction times"""
    global session_data
...
def detect_persons_with_attention(image, mode="image"):
    """Detect persons with attention tracking"""
    ...
    if detection_results.detections:
        for i, detection in enumerate(detection_results.detections):
            ...
            if mode == "video" and is_monitoring_active:
                ...
                should_trigger, is_reminder = should_trigger_alert(person_id, status_text, session_duration)
                if should_trigger:
                    print(f"Triggering alert for Person {person_id} - {status_text} - Duration: {session_duration:.1f}s")
                    trigger_alert(person_id, status_text, session_duration, is_reminder)
...
def generate_live_pdf_report(session_data, output_path):
    ...
    try:
        ...
        initial_alerts = {'SLEEPING': 0, 'YAWNING': 0, 'NOT FOCUSED': 0}
        reminder_alerts = {'SLEEPING': 0, 'YAWNING': 0, 'NOT FOCUSED': 0}
...
def generate_upload_pdf_report(detections, file_info, output_path):
    """Generate PDF for upload analysis"""
    doc = SimpleDocTemplate(output_path, pagesize=A4)
    styles = getSampleStyleSheet()
    story = []
...
def start_monitoring():
    ...
    try:
        ...
        with monitoring_lock:
            ...
            print(f"Session started: {session_data['start_time']}")
...
def stop_monitoring():
    ...
    try:
        ...
        with monitoring_lock:
            ...
            for person_key, current_state in person_current_states.items():
                if current_state and current_state in DISTRACTION_THRESHOLDS:
                    if person_key in person_state_start_times:
                        ...
                        print(f"Finalized {current_state} session: {session_duration:.2f}s")
            ...
            print(f"Session stopped: {session_data['end_time']}")
            ...
            try:
                ...
                if len(session_data.get('recording_frames', [])) > 0:
                    ...
                    if video_result and os.path.exists(recording_path):
                        ...
                        print(f"Video generated: {os.path.basename(recording_path)}")
                    else:
...
def get_monitoring_data():
    ...
    try:
        ...
        print(f"Get monitoring data error: {str(e)}")
        traceback.print_exc()
...
def report_file(filename):
    ...
    try:
        ...
        print(f"Report serve error: {str(e)}")
...
def recording_file(filename):
    ...
    try:
        ...
        print(f"Recording serve error: {str(e)}")
...
def detected_file(filename):
    ...
    try:
        ...
        print(f"Detected file error: {str(e)}")
...
if __name__ == "__main__":
    try:
        if not init_mediapipe():
            print("WARNING: MediaPipe initialization failed")
        ...
        print(f"Frame storage: every {FRAME_STORAGE_INTERVAL} frames, max {MAX_STORED_FRAMES}")
