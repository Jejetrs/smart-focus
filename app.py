...
# Configuration dengan path yang lebih robust untuk Railway
...
# Session tracking untuk file yang dibuat
...
def create_session_recording_from_frames(recording_frames, output_path):
    """Create video recording from captured frames"""
    try:
...
def create_demo_recording_file(output_path):
    """Create a demo recording file when no frames are captured"""
    try:
        fourcc = cv.VideoWriter_fourcc(*'mp4v')
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            print(f"Demo recording created: {output_path}")
            return output_path
        else:
...
def ensure_session_data():
    ...
    if current_session_files['pdf_path'] and os.path.exists(current_session_files['pdf_path']):
        try:
            os.remove(current_session_files['pdf_path'])
            print(f"Cleaned up old PDF: {current_session_files['pdf_path']}")
        except Exception as e:
            print(f"Error cleaning up old PDF: {str(e)}")
    
    # Remove old video file if exists
    if current_session_files['video_path'] and os.path.exists(current_session_files['video_path']):
        try:
            os.remove(current_session_files['video_path'])
            print(f"Cleaned up old video: {current_session_files['video_path']}")
        except Exception as e:
            print(f"Error cleaning up old video: {str(e)}")
...
def report_file(filename):
    """Fixed route untuk reports - langsung dari /reports/"""
    try:
...
def recording_file(filename):
    """Fixed route untuk recordings - langsung dari /recordings/"""
    try:
...
def report_file_backup(filename):
    return report_file(filename)

@application.route('/static/recordings/<filename>')
def recording_file_backup(filename):
    return recording_file(filename)
            ...
            if file_ext in ['jpg', 'jpeg', 'png', 'bmp']:
                ...
                if image is not None:
                    ...
                    if pdf_result and os.path.exists(pdf_path):
                        result["pdf_report"] = f"/reports/{pdf_filename}"
...
def stop_monitoring():
    ...
    try:
        if session_data.get('recording_frames') and len(session_data['recording_frames']) > 0:
            video_result = create_session_recording_from_frames(session_data['recording_frames'], video_path)
            print(f"Created video from {len(session_data['recording_frames'])} frames")
        else:
            video_result = create_demo_recording_file(video_path)
            print("Created demo video (no frames captured)")
        
        if video_result and os.path.exists(video_path):
            current_session_files['video_path'] = video_path
            current_session_files['video_filename'] = video_filename
            session_data['recording_path'] = video_path
            print(f"Video recording created: {video_path}")
        else:
            print("Failed to create video recording")
    except Exception as e:
        print(f"Error creating video recording: {str(e)}")
    
    # Build response
    response_data = {
        "status": "success", 
        "message": "Monitoring stopped and files generated successfully"
    }
    
    # Add PDF report URL if available
    if current_session_files['pdf_path'] and os.path.exists(current_session_files['pdf_path']):
        response_data["pdf_report"] = f"/reports/{current_session_files['pdf_filename']}"
        print(f"PDF report URL: {response_data['pdf_report']}")
    
    # Add video recording URL if available
    if current_session_files['video_path'] and os.path.exists(current_session_files['video_path']):
        response_data["video_file"] = f"/recordings/{current_session_files['video_filename']}"
        print(f"Video recording URL: {response_data['video_file']}")
    
    print(f"Session stopped. Generated files: PDF={current_session_files['pdf_filename']}, Video={current_session_files['video_filename']}")
    return jsonify(response_data)
...
def process_frame():
    ...
    try:
        ...
        # Save frame for recording if monitoring is active
        ...
        # Convert processed frame to base64 for frontend
...
@application.route('/get_current_session_files')
def get_current_session_files():
    """Endpoint untuk mendapatkan file session saat ini"""
    global current_session_files
    
    files = {}
    
    if current_session_files['pdf_path'] and os.path.exists(current_session_files['pdf_path']):
        files['pdf_report'] = f"/reports/{current_session_files['pdf_filename']}"
    
    if current_session_files['video_path'] and os.path.exists(current_session_files['video_path']):
        files['video_file'] = f"/recordings/{current_session_files['video_filename']}"
...
def health_check():
    return jsonify({
        ...
        "current_session": {
            ...
            "pdf_available": current_session_files['pdf_path'] is not None and os.path.exists(current_session_files['pdf_path']) if current_session_files['pdf_path'] else False,
            "video_available": current_session_files['video_path'] is not None and os.path.exists(current_session_files['video_path']) if current_session_files['video_path'] else False
        }
    })
